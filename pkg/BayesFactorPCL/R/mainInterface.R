...
BFAnovaGUI <- function(data=NULL,filename=NULL,setup=NULL,skipInstr=FALSE,name="Analysis",interface=TRUE,doOutput=TRUE, storePred=FALSE){
  ...
  BFAnovaInit()
  passedPackage = class(setup)=="PCLBFAnovaPackage"
  pack = new("PCLBFAnovaPackage")
  ...
  if(passedPackage){
    ...
    pack@settings@intMods     = pack@settings@SelEffs$SelEffs[pack@settings@SelEffs$SelEffs[,5]==TRUE,1:2]
    pack@settings@SelEffs2    = pack@settings@SelEffs$SelEffs[pack@settings@SelEffs$SelEffs[,5]==TRUE,]
   ...
   pack@settings@namedDat2   = createMeaningfulCols(pack@settings@newDat,pack@settings@mods,pack@settings@intMods,pack@settings@SelCols$selectedcols)
    ...
    pack@settings@Lvls        =  apply(pack@settings@newDat2[,-1],2,function(v) length(unique(v)))
                             
    
      #pack@settings@PriorSetup = BFAnovaPriorSetup(TRUE)	
  ...
  if(FALSE){
    if(interface){
      ...
      out = doModel(pack@settings@designMatrix,pack@settings@newDat2[,1],pack@settings@Lvls)
