...
as.vector(anovaBF(x~grp, data=dat,progress=FALSE))
as.vector(generalTestBF(x~grp, data=dat,progress=FALSE))
```
...
idOnly = lmBF(x~id, data=dat, whichRandom="id", progress=FALSE)

summary(lm(x~id+grp,data=dat))

```

If the prior settings are consistent, these two numbers should be almost the same (within MC estimation error).
```{r}
as.vector(lmBF(x ~ grp+id, data=dat, whichRandom="id", progress=FALSE)/idOnly)
as.vector(lmBF(x ~ forReg+id, data=dat, whichRandom="id", progress=FALSE)/idOnly)
```
